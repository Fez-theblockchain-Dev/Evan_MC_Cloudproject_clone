...
def _handle_create_upload(event):
    ...
    try:
        ...
        return _response(500, {"message": "Failed to create job record. Internal server failure"})
    
    # new error handeling msg for imporper upload from client side "bad request" : 400
    try: 
        jobs_table.put_item(Item=item)
    except ClientError as e:
        print(f"attachment not compatable. Please convert file to either PDF, DOC, DOCX): {e}")

        return _response(400, {"message: file tyoe uploaded not compatable. Please upload S3 accepted files only"})
